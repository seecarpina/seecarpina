...
const form = document.getElementById("formSolicitacaoLimpeza");
...
function abrirDetalhesSolicitacao(solicitacaoId) {
  ...
  tituloDrawerDetalhes.textContent = "Materiais de limpeza e higiene";
  ...
  tipoDetalhes.textContent = "Materiais de limpeza e higiene";
...
function adicionarMaterialSolicitacao() {
  ...
  if (!material) {
    mostrarNotificacao("Selecione um material de limpeza ou higiene.");
...
async function gerarProtocoloLimpeza() {
  const ano = new Date().getFullYear();

  const contadorRef = ref(
    rtdb,
    `portalGestor/solicitacoes/contadores/materiaisLimpeza/${ano}`,
  );
  ...
  return `LIM-${ano}-${String(numero).padStart(4, "0")}`;
}
...
async function salvarSolicitacao() {
  ...
  const protocolo = await gerarProtocoloLimpeza();
  ...
  const solicitacao = {
    modulo: "MATERIAIS_LIMPEZA",
    protocolo,

    tipo: "MATERIAIS_LIMPEZA",
    tipoNome: "Materiais de limpeza e higiene",
    ...
    historico: {
      [historicoId]: {
        ...
        descricao:
          `Solicitação de materiais de limpeza e higiene enviada ` +
